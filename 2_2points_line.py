...
def generate_problem():
    ...
    # dx の候補（最大5以内）
    if isinstance(m, int):
        dx_candidates = [k for k in range(-5, 6) if k not in (0,)]
    else:
        q = m.denominator
...
if st.button("判定する"):
    if selected is None:
        ...
        # 誤答の直線
        if selected != correct:
            sel = selected.replace("$", "").replace("y = ", "")
            m_s, b_s = sel.split("x")
            m_s = Fraction(m_s.replace("\\frac", "").replace("{", "").replace("}", "").replace(" ", ""))
            b_s = Fraction(b_s.replace("+", "").replace("\\frac", "").replace("{", "").replace("}", "").replace(" ", ""))
